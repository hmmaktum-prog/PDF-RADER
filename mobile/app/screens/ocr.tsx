...
import { ensureEngine, mergeBoxesIntoBlocks } from '../utils/paddleOcrService';
...
export default function OcrScreen() {
  ...
  const handleDownloadPack = async (langId: string): Promise<void> => {
    ...
    try {
      const ok = await downloadOcrModel(langId, (pct: number) => {
        setDownloadProgress(Math.round(pct));
      });
      if (!ok) throw new Error('Model download returned false');
      setDownloadedPacks(prev => ({ ...prev, [langId]: true }));
      Alert.alert('Download Complete',
        `${LANGUAGES.find(l => l.id === langId)?.label} offline pack installed successfully.`);
    } catch (e: any) {
      Alert.alert('Download Failed', e?.message || 'Could not install offline pack.');
    } finally {
  ...
  const handleOcr = async (
    ...
    try {
      ...
      if (useGemini) {
        ...
        onProgress(42, 'Initializing PaddleOCR engine...');

        // Ensure OCR engine is initialized for selected language
        const engineOk = await ensureEngine(language);
        if (!engineOk) {
          const langLabel = LANGUAGES.find(l => l.id === language)?.label || language;
          throw new Error(`OCR engine failed to initialize for '${langLabel}'. Please re-download the offline pack.`);
        }
        onProgress(45, 'Engine ready. Starting recognition...');
        ...
        for (let i = 0; i < renderedPages.length; i++) {
          ...
          // Cleanup rendered image after processing to save disk space
          try { await FileSystem.deleteAsync(renderedPages[i], { idempotent: true }); } catch {}
          finalResult = result;
          ...
          // Use the semantic blocks generated by our new heuristics
          const pageBlocks = mergeBoxesIntoBlocks(result.boxes);
          blocks.push(...pageBlocks);

          if (pageNum < renderedPages.length) {
            blocks.push({ type: 'separator', content: '--- Page Break ---' });
          }
        }

        // Build combined result
        finalResult = finalResult
          ? { ...finalResult, fullText: combinedText, tables: allTables, formulas: allFormulas }
          : null;
